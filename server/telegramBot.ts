...
Пример: /aivideo crypto trading success story`;
    ...
    try {
      await bot!.sendMessage(chatId, '🎬 Генерирую AI видео... (10-20 секунд)\n\n✨ Используется Hugging Face LTX-Video\n💰 Стоимость: $0.00 (бесплатно!)');
...
🚀 Видео будет опубликовано в следующем сообщении...`, { parse_mode: 'HTML' });
        ...
        await bot!.sendMessage(chatId, '⏳ Видео генерируется... Это может занять до 30 секунд.\n\nПопробуйте команду ещё раз через минуту или используйте веб-интерфейс для более стабильной генерации.');
      }
    } catch (error) {
      console.error('Ошибка генерации AI видео:', error);
      await bot!.sendMessage(chatId, `❌ Ошибка генерации видео

💡 <b>Альтернативные варианты:</b>
1. Попробуйте ещё раз через минуту
2. Используйте веб-интерфейс: /menu → AI Video Studio
3. Загрузите своё видео: /uploadvideo

<b>Причины ошибки:</b>
• Модель может загружаться (подождите 20 сек)
• Попробуйте более простое описание
• Используйте английский язык`, { parse_mode: 'HTML' });
    }
  });
